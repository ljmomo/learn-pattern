...
修改前生日对象对象：true
修改前Phone对象对象：true
修改后原  名称：张三  年龄：25 生日：Sun Jan 18 22:23:03 CST 1970 手机：null
修改后克隆 名称：李四  年龄：26 生日：Mon Jan 19 01:09:44 CST 1970 手机：null
修改后生日对象对象：false
```
发现克隆后 prototype和prototype1 不是同一个对象，但是日期对象和Phone克隆前和克隆后是同一个对象。这样就不科学了，克隆后日期和Phone和原来的对象不一样才对呀！我们重新修改了克隆后的值发现日期对象修改后又不会影响克隆前的对象 感觉非常奇怪！百思不得其解，两个对象一样不是应该修改日期后两个值都改变！并且java克隆是浅克隆呀！符合这个原理呀！日期对象相同，但是改了值为啥没有同时变呢！ 然后我又重新做了一个实验！ 
...
```
克隆后和克隆前的对象对比：false
克隆后和克隆前ArrayList对象对比：true
克隆对象ArrayList添加一个值：[12, 222]
被克隆对象ArrayList：[12, 222]
```
结果发现比较符合java Cloneable 的逻辑！

如果才能保证引用对象克隆后不是同一个对象呢！这样就引入了深度克隆！下面演示深度克隆！
...
修改后克隆 名称：张三  年龄：25 生日：Mon Jan 19 01:09:44 CST 1970 手机：Phone{                            name:'HUA WEI',                             ys:'BLUE'}
修改后生日对象对象：false
```
